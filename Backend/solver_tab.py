...
class _SolveWorker(QObject):
    ...
    def _run_2d(self, snapshot, progress_callback):
        """Run exactly the selected 2-D scattering mode.

        Keeping this dispatch shared by the ordinary and mesh-convergence
        paths prevents a bistatic request from being replaced by a
        monostatic solve when constructing the base or fine result.
        """
    ...
    def run(self):
        try:
            if self.solver_kind == "bor":
                ...
                result = self._run_2d(self.snapshot, base_cb)
                ...
                fine_result = self._run_2d(fine_snapshot, fine_cb)
                mesh_gate = evaluate_mesh_convergence(
                    base_result=result,
                    fine_result=fine_result,
                    rms_limit_db=self.mesh_rms_limit_db,
...
class SolverTab(QWidget):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        # Interactive solves export by default, so their quality gate must
        # also default to fail-closed. Disabling it is an explicit diagnostic
        # choice and the returned result remains visibly marked.
        ...
        self.cmb_solver_method.setToolTip(
            "Auto and Direct use dense direct linear algebra.\n"
            "Explicit FMM is supported only for TE Robin or multi-region "
            "formulations. Unsupported FMM requests reject the solve instead "
            "of falling back to dense."
        )
    ...
    def _set_solving_state(self, solving: bool):
        ...
        enable_2d_quality = not solving and not is_bor
        ...
        self.chk_strict_quality.setEnabled(enable_2d_quality)
        self.edit_quality_residual_max.setEnabled(enable_2d_quality)
        self.edit_quality_condition_max.setEnabled(enable_2d_quality)
        self.edit_quality_warnings_max.setEnabled(enable_2d_quality)
