    ...
    def from_entries(
        ...
        for row in ibcs_entries:
            ...
            if flag > 50:
                if len(row) != 1:
                    raise ValueError(
                        f"Tabulated IBC flag {flag} must contain only the flag; "
                        f"got {len(row)} fields.")
        ...
        for row in dielectric_entries:
            ...
            if flag > 50:
                if len(row) != 1:
                    raise ValueError(
                        f"Tabulated dielectric flag {flag} must contain only "
                        f"the flag; got {len(row)} fields.")
                path = _resolve_mat_file(base_dir, flag)
...
def _resolve_mat_file(base_dir: str, flag: int) -> str:
    """Resolve a mat.<flag> only relative to the declared geometry directory."""
...
def _material_base_dir_for_snapshot(
    ...
    """Return the single declared directory used for ``mat.N`` sidecars.
...
def evaluate_quality_gate(
    ...
    return {
        ...
        "reason": "; ".join(violations) if violations else "quality thresholds satisfied",
    }
...
def solve_bistatic_rcs_2d(
    ...
    for freq_ghz in frequencies:
        ...
        if use_te_robin_mfie:
            ...
            mfie_alpha_nodes = _te_robin_alpha_nodes(mesh, coupled_infos, pol)
        ...
        if compute_condition_number:
            ...
            if diagnostic_matrix is not None:
                cond_values.append(_cond_estimate(diagnostic_matrix))
        ...
        for inc_deg in inc_angles:
            ...
            if use_sheet or use_mixed_sheet:
                ...
                condition_diagnostics = (
                    {}
                    if compute_condition_number
                    and not frequency_condition_recorded
                    else None
                )
                ...
                if compute_condition_number and not frequency_condition_recorded:
                    cond_values.append(_cond_estimate(a_sys))
                    frequency_condition_recorded = True
