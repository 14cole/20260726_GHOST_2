...
def _validate_dielectric_row(tokens: List[str], lineno_for_err: str) -> None:
    """Validate one strict inline or tabulated dielectric definition."""
    if not tokens:
        return
    try:
        flag = int(tokens[0])
    ...
    if flag > TABULATED_FLAG_THRESHOLD:
        if len(tokens) != 1:
            raise ValueError(
                f"Tabulated dielectric row "
                f"(flag>{TABULATED_FLAG_THRESHOLD}) must be a single flag "
                f"token; got {len(tokens)}: {lineno_for_err}")
        return
    if len(tokens) != 5:
        raise ValueError(
